...
const frontmatter = [
  ...
  '# TODO populate verificationLog during research. EACH entry MUST include',
  '# verifiedViaFetch (REQUIRED). Set verifiedViaFetch:true only for entries',
  '# representing an actual fetch (URL opened + quoted text diffed against',
  '# source). Lexical / interpretive / paraphrase / self-quote claims use',
  '# verifiedViaFetch:false. Per AUTHORING §6.0, every cross-reference',
  '# verbatim quote >6 words requires a verifiedViaFetch:true entry — the',
  '# lint:quotation-fidelity rule blocks otherwise.',
  'verificationLog: []',
  'status: draft',
  `draftedBy: ${draftedBy}`,
  `draftedOn: "${today}"`,
  '# quotationFidelity: \'enforced\' (default for new chapters) opts into',
  '# full ERROR-level enforcement of lint:quotation-fidelity. Existing',
  '# chapters drafted before the rule shipped are marked \'legacy\' as a',
  '# one-time grandfather. New chapters do not need this field; the default',
  '# (enforced) applies. Setting \'legacy\' explicitly is reserved for',
  '# pre-rule chapters and is honest about discipline-not-yet-applied.',
