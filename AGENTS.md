...
SumBible is a public scriptural-reference site for readers who want concise orientation and substantive, source-grounded chapter, book, and canon summaries. Each entry pairs a short highlight with a deeper MDX summary. The site is faith-respecting and explicitly Latter-day Saint in its treatment of the LDS Standard Works while remaining academically honest about genuine scholarly disagreement in Bible studies.
...
Repository at a glance: Astro 6, MDX content collections, TypeScript, Zod 4 schemas, Tailwind CSS 4, Pagefind search, and Vercel deployment. Content lives under `src/content/`; `src/content.config.ts` is the source of truth for frontmatter; reusable MDX components live under `src/components/`; authoring, validation, and generated-context tooling lives under `scripts/`.
...
For Bible content, use a scholarly-spectrum voice: where genuine believer-level disagreement exists, report the spectrum and do not arbitrate. For the Book of Mormon, Doctrine and Covenants, and Pearl of Great Price, use a believing-LDS voice. Treat their canonical claims as matter of fact rather than distancing them with phrases such as “the text claims” or “the Church teaches.” Use the KJV as the primary biblical text.
...
Capitalize every reference to Deity in SumBible prose: God, Lord, Father, Son, Holy Ghost, Holy Spirit, Spirit, Savior, Redeemer, Creator, Author, Almighty, Most High, Ancient of Days, Word, Beloved, Messiah, Christ, Jesus, Yahweh, Jehovah, and the pronouns He, Him, His, and Himself. Context governs terms such as spirit, father, lord, and son when they refer to created beings. Preserve the capitalization of quoted scripture exactly as published. In particular, the KJV expression “my lord” remains lowercase when addressed to a non-Deity figure such as Pharaoh, Laban, Esau, or another earthly superior; do not “correct” it to Deity capitalization.

## §3 — The §6.0 Fetch-and-Diff Guard

This is the most important behavioral rule in this file. It prevents a proven systemic failure: before this guard existed, integrity sweeps found an error rate of 35% in Standard Works cross-reference quotations and 13% in Bible cross-reference quotations. Agents had reconstructed wording from memory while verification logs falsely appeared to certify URLs that had not actually been opened and compared.

Any verbatim scripture quotation longer than approximately six words from a book OTHER than the chapter's primary subject MUST be fetch-verified against the actual source text at the time of writing. Never quote such a passage from memory. Its `verificationLog` entry must contain `verifiedViaFetch: true` and the actual URL opened and used for the comparison. A URL recorded without opening and diffing it is not verification.

Chapter self-quotes—where the chapter quotes its own primary verses—are exempt and honestly use `verifiedViaFetch: false`. When uncertain, paraphrase. Paraphrase is the preferred default; verbatim quotation is the exception, reserved for cases where exact wording carries the argument.
...
Runs the `prebuild` generators for cross-reference indexes and book contexts, then performs the Astro production build, creates the Pagefind index, and copies Pagefind assets. Passing means the command exits 0 with the site built successfully and no generation or build error.

```bash
npm run lint:content
```

Runs the repository's content-discipline checks: highlight shape, source coverage, quoted dates, raw references, status integrity, Christ-reference integrity, verification-log density, schema relationships, Deity capitalization, editorial hedges, repetition, and cross-reference quotation fidelity. Passing means it exits 0 and its summary reports `0 error(s)`; warnings still require human review and must not be ignored mechanically.
...
HALT if the build fails irrecoverably, lint will not pass after reasonable attempts, a required source cannot be fetched, or structural ambiguity requires human judgment. Do not bury the problem or weaken a check.

Commit everything that is clean, push the session branch, and write the reason to `SESSION_<N>_HALT.md`. Open a PR whose title begins `[PARTIAL]` and make the halt reason prominent in both title and body. A clean partial is always better than a complete batch with buried errors.

## §7 — Session Scope Rules

Draft chapters sequentially only. Never use parallel subagents for chapter drafting. Cross-chapter consistency depends on each completed chapter and regenerated book context being visible to the next chapter before drafting continues; parallel drafts cannot reliably inherit one another's decisions, themes, verified references, and corrections.

Use a single push at session close to protect the Vercel Hobby-tier budget of 100 deployments per day. For normal content sessions, branch from `dev` and open a PR back to `dev`. Never commit directly to `main`. Feature work belongs on its session branch and is merged only through human-reviewed PRs.
...
- The current harness session branch is `pi/agents-harness`, created from `main` as explicitly requested for this administrative task. A future chapter-drafting session must follow §7: create a new `pi/...` session branch from `dev` and open its PR back to `dev`; do not reuse this harness branch.
...
If the voice starts to drift, stop immediately and re-read `src/content/chapters/bible-ot/01-genesis/01.mdx` before continuing. That chapter is the permanent voice reference for the entire project and the calibration sample against which future chapters are measured.
